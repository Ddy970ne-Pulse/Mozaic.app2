...
const OnCallSchedule = ({ user }) => {
  ...
  const fetchOnCallSchedule = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // API pas encore implémentée, utiliser données mockées
        console.log('API non disponible, utilisation données mockées');
        setOnCallData([]);
      }
    } catch (error) {
  ...
  // Créer une astreinte (version simplifiée sans API backend)
  const handleQuickAddSubmit = async () => {
    ...
    try {
      ...
      // Pour l'instant, ajouter localement (en attendant l'API backend)
      const startDate = new Date(quickAddData.date);
      const newOnCallItems = [];
      
      if (quickAddData.type === 'semaine') {
        // Ajouter 7 jours (dimanche à samedi)
        for (let i = 0; i < 7; i++) {
      ...
      alert(`✅ Astreinte créée localement pour ${employee?.name || employee?.prenom}\n\n⚠️ Note: L'API backend doit être implémentée pour une persistance complète`);
  ...
  const handleDelete = async () => {
    ...
    alert('✅ Astreinte supprimée localement\n\n⚠️ Note: L\'API backend doit être implémentée');
            ...
            {calendarDays.map((dayData, index) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Bouton + pour ajout rapide (admin uniquement, case vide du mois courant) */}
